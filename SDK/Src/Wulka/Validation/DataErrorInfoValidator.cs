...
{
	/// <summary>
	/// This code comes from the interweb: http://weblogs.asp.net/marianor/archive/2009/04/17/wpf-validation-with-attributes-and-idataerrorinfo-interface-in-mvvm.aspx
	/// </summary>
	/// <typeparam name="T"></typeparam>
	public static class DataErrorInfoValidator<T> where T : IDataErrorInfo
	{
		private static readonly Dictionary<string, KeyValuePair<Func<T, object>, ValidationAttribute[]>> mAllValidators;

		static DataErrorInfoValidator()
		{
		...
		public static string Validate(T source, string columnName)
		{
			...
			if (mAllValidators.TryGetValue(columnName, out validators))
			{
				...
				string[] errors = validators.Value.Where(v => !v.IsValid(value)).Select(v => v.ErrorMessage ?? "").ToArray();
